document.addEventListener("DOMContentLoaded", function () {
  ...
  const lucine = document.querySelectorAll(".space");
  const allStelle = [];
  const allStelleWithFeedback = [];
  ...
  lucine.forEach((stella) => {
    allStelle.push(stella);
    stella.onmouseover = (event) => {
      allStelleWithFeedback.length = 0;

      allStelle.forEach((s) => {
        s.classList.remove("glow");
      });
      for (let i = 0; i <= allStelle.indexOf(stella); i++) {
        allStelle[i].classList.add("glow");
        allStelleWithFeedback.push(stella);
      }
    };
  });
  console.log(allStelleWithFeedback);
